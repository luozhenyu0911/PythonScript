...
'''
parser = argparse.ArgumentParser(description="This tool creates Z-scores of read counts and profile plot over sets of genomic regions. Typically, these regions are genes, but any other regions defined in BED will work.", 
...
group.add_argument('--referencePoint',type=str,help="<referencePoint {start/end/center}>"+"\t"+"plot the distribution of the reads at reference point from the genome. This option is mutually exclusive with \"--scaleRegion\".",metavar='')
...
parser.add_argument('--matrixRowID',action='store_true',help="<Optional argument>"+"\t"+"Use 4th column of bed file as the matrix ROW ID. (Default=False)",default=False)
            ...
            for x in range(start,end+reside,blockSize):
                for y in range(x,(x+blockSize)):
                    if str(y) in readdict[chr]:
                        ...
                        pass
                sublist.append((sumReads*10000000)/(count*blockSize))
                sumReads=0
...
def Genomewide_zscore_with_control(bamCoverage,controlCoverage,window,chr,chrlen,count):
    treat_rpkm = []
    input_rpkm = []
    for a,b in zip(chr,chrlen):
        if str(a) in bamCoverage:
            for binStart in range(0,int(b),window):
                sumTreatReads=0
                sumInputReads=0
                for j in range(binStart,(binStart+window)):
                    #此处预先判断染色体有没有在字典里
                    #数ChIP的reads
                    if str(j) in bamCoverage[str(a)]:
                        sumTreatReads+=bamCoverage[str(a)][str(j)]
                    else:
                        pass
                    #数input的reads
                    if str(j) in controlCoverage[str(a)]:
                        sumInputReads+=controlCoverage[str(a)][str(j)]
                    else:
                        pass
                treat_rpkm.append((sumTreatReads*10000000)/(count*window))
                input_rpkm.append((sumInputReads*10000000)/(count*window))
    normed = np.array(treat_rpkm)-np.array(input_rpkm)
    normed[normed<0]=0
    return [np.mean(normed),np.std(normed)]

def Genomewide_zscore(bamCoverage,window,chr,chrlen,count):
    treat_rpkm = []
    for a,b in zip(chr,chrlen):
        if str(a) in bamCoverage:
            for binStart in range(0,int(b),window): 
                ...
                for j in range(binStart,(binStart+window)):
                    ...
                    if str(j) in bamCoverage[str(a)]:
                        ...
                        pass
                treat_rpkm.append((sumTreatReads*10000000)/(count*window))
    return [np.mean(treat_rpkm),np.std(treat_rpkm)]
    ...
    if facet=="Bam":
        ...
        if len(u)>1:
            ...
            if scaleY:
                ...
                min_y=min_value-(max_value-min_value)*0.05
...
if Inputbamlist:
    ...
    if referencePoint:
        ...
        if referencePoint=="start":
            ...
            #fullmat header
            header = ["Bamlabel","Bedlabel","ID"]+["bin"+str(bin+1) for bin in range(NumOfBin)]
            ...
            for Bam in range(len(bamlist)):
                ...
                #计算bam文件的reads平均值和标准差
                Chr = bamf.references
                Chrlen = bamf.lengths
                bamAvg,bamSd = Genomewide_zscore_with_control(bamrCoverageDict,controlCoverageDict,wdsize,Chr,Chrlen,bamTotalReads)
                for Bed in range(len(bedlist)):
                    ...
                    tup[tup>q]=q
                    tup=np.array([ [(i-bamAvg)/bamSd for i in l] for l in tup ])
                    tup_avg=tup.mean(axis=0)
                    Values=list(tup)
                    if args.matrixRowID:
                        ID=pd.read_csv(bedlist[Bed],sep='\t',header=None).iloc[:,3].tolist()
                        for index in range(len(values)):#print matrix
                            print(Bamlabels[Bam],Bedlabels[Bed],ID[index],*Values[index],sep="\t",end="\n",file=out_matrix)
                    else:
                        for index in range(len(values)):#print matrix
                            print(Bamlabels[Bam],Bedlabels[Bed],*Values[index],sep="\t",end="\n",file=out_matrix)
                    i=1
                    for binValue in tup_avg:#print average bin value
                ...
                for Bed in range(len(bedlist)):
                    ...
                    if args.matrixRowID:
                        ...
                        for index in range(len(values)):#print matrix
                            print(Bamlabels[Bam],Bedlabels[Bed],*Values[index],sep="\t",end="\n",file=out_matrix)
                    i=1
                    for binValue in tup2:#print average bin value
                        print(Bamlabels[Bam],Bedlabels[Bed],i,binValue,sep="\t",end="\n",file=out_profile)
                        i+=1
            out_matrix.close()
            out_profile.close()
            ...
            for Bam in range(len(bamlist)):
                ...
                for Bed in range(len(bedlist)):
                    ...
                    Invalues=bedCountCenter(bedlist[Bed],controlCoverageDict,controlTotalReads,wdsize,up,down)
                    tup1=np.array(values)
                    Intup=np.array(Invalues)
                    tup=tup1-Intup
                    tup[tup<0]=0
    ...
    if referencePoint:
        ...
        if referencePoint=="start":
            ...
            for Bam in range(len(bamlist)):
                ...
                Chr = bamf.references
                Chrlen = bamf.lengths
                bamAvg,bamSd = Genomewide_zscore(bamrCoverageDict,wdsize,Chr,Chrlen,bamTotalReads)
                for Bed in range(len(bedlist)):
                    values=bedCount(bedlist[Bed],bamrCoverageDict,bamTotalReads,wdsize,up,down)
                    tup1=np.array(values)
                    #去掉极值
                    ...
                    tup1[tup1>q]=q
                    tup1=np.array([ [(i-bamAvg)/bamSd for i in l] for l in tup1 ])
