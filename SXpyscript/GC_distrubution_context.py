...
def message():
    ...
    print("Description:"+"\n"+"\t"+"\t"+"This tool creates GC distribution profile over sets of genomic regions."+"\n")
    print("Options:"+"\n")
    ...
    print("\t"+"-C"+"    <Context>"+"\t"+"GC context,could be CG, CHG or CHH.")
...
opts,args = getopt.getopt(sys.argv[1:],'-h-v-b:-G:-u:-r:-d:-w:-N:-o:-l:-F:-C:',['help','version','scale-regions','reference-point'])
for opt_name,opt_value in opts:
    if opt_name in ('-h','--help'):
        message()
    if opt_name in ('-v','--version'):
        print("**.py 1.0 2019-10-29")
    if opt_name in ('-b'):
        fasta = opt_value
        #CGmapfile = opt_value
    ...
    if opt_name not in ('-h,--help,-v,--version,-b,-G,-r,-u,-d,-w,-N,-o,-l,-F,-C,--scale-regions,--reference-point'):
        message()
    ...
    for line in bedf:
        ...
        if line[5] is "+":
            ...
            for k in range(left,right,windows):
                ...
                num = len(re.findall(condition,seq,overlapped=True))
                Revnum = len(re.findall(condition,RevSeq,overlapped=True))
                total = num+Revnum
                #GC=num/windows
    ...
    for line in bedf:
        ...
        if line[5] is "+":
            ...
            for k in range(int(line[2])+up,int(line[2]),-windows):
                seq=fasta.sequence({'chr':line[0],'start':k-windows,'stop':k},one_based=False)
                RevSeq=Revcomplement(seq)
                num = len(re.findall(condition,RevSeq,overlapped=True))
                GC=num/windows
                sublist.append(GC)
...
if "--reference-point" in Option:
    ...
    if "-r" in Option and str(referencePoint)=="start":
        ...
        plt.ylabel(Context+" distribution")
