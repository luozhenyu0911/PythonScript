...
# class MethyDict2Instance:
#     __slots__ = 'CG','CHG','CHH'
...
def CGmapTodict(Input,Context,minimalCoverage):
    region={}
    #跳过表头
    next(Input)
    for line in Input:
        ...
        if context == Context:
            ...
            pass
    #字典转换为实例
    #region=MethyDict2Instance(CG,CHG,CHH)
    return region
    ...
    if referencePoint=="start":
        ...
        for Context in ['CG','CHG','CHH']:
            for Bam in range(len(bamlist)):
                ...
                MethyDict = CGmapTodict(bamf,Context,minCov)
                bamf.close()
                for Bed in range(len(bedlist)):
                    values=bedMethyStart(bedlist[Bed],MethyDict,wdsize,up,down)
        ...
        for Context in ['CG','CHG','CHH']:
            for Bam in range(len(bamlist)):
                ...
                bamf.close()
                for Bed in range(len(bedlist)):
                    values=bedMethyEnd(bedlist[Bed],MethyDict,wdsize,up,down)
                    tup1=np.array(values)
                    tup2=np.nanmean(tup1,axis=0)
                    #print matrix
                    for index in range(len(values)):
...
#print("Usage: python tmp1.py <<--scale-regions or --reference-point>> -b <bamfile> -G <bedfile> -g <genelist> -r <referencePoint {start or end}> -u <upstream_distance> -d <downstream_distance> -w <window_size> -bn <Equal_block_number> -o <outputfile>")
    ...
    for context in ['CG','CHG','CHH']:
        for Bam in range(len(bamlist)):
            ...
            bamf.close()
            for Bed in range(len(bedlist)):
                values=bedMethyBody(bedlist[Bed],MethyDict,wdsize,up,down,blocknum)
                tup1=np.array(values)
                tup2=np.nanmean(tup1,axis=0)
                #print matrix
                for index in range(len(values)):
