...
example_text = '''example:
 2020.6.14
 python **.py --referencePoint start/end -b CGmap.gz,... -G **.bed,**.bed... -L bamlabel1,bamlabel2 -l bedlabel1,bedlabel2 --Facet Bam/Bed -u 1000 -d 1000 -w 50 -o prefix
 python **.py --scaleRegion -b CGmap.gz,... -G **.bed,**.bed... -L bamlabel1,bamlabel2 -l bedlabel1,bedlabel2 --Facet Bam/Bed -u 1000 -d 1000 -w 50 -N 100 -o prefix
'''
...
def bedMethyCenter(bedfile,readdict,windows,up,down):
    metC=0
    sumC=0
    methyList=[]
    bedf=open(bedfile,'r')
...
if referencePoint:
    ...
    if referencePoint=="start":
        ...
        for Bam in range(len(bamlist)):
            ...
            for Bed in range(len(bedlist)):
                for context in ['CG','CHG','CHH']:
                    values=bedMethyCenter(bedlist[Bed],MethyDict[context],wdsize,up,down)
                    tup1=np.array(values)
                    tup2=np.nanmean(tup1,axis=0)
                    #print matrix
                    for index in range(len(values)):
