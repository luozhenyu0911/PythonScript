...
example_text = '''example:
 2021.3.25
 python **.py --referencePoint start/end/center -b **.fasta -G **.bed,**.bed... --bedlabeltxt **.txt,**.txt... -u 1000 -d 1000 -w 50 -l **,** -o **.mean,**.matrix -f **.pdf
 python **.py --scaleRegion -b **.fasta -G **.bed,**.bed... --bedlabeltxt **.txt,**.txt... -u 1000 -d 1000 -w 50 -N 100 -l **,** -o **.mean,**.matrix -f **.pdf
'''
parser = argparse.ArgumentParser(description="This tool creates Dinucleotide density profile over sets of genomic regions.", 
...
parser.add_argument('--fasta','-b',type= str,help="<fasta>"+"\t"+"Reference genome fasta files.",required= True,metavar='')
...
if args.bedlabel:
    bedlabel=args.bedlabel.split(",")
else:
    bedlabel=[]
    with open(args.bedlabeltxt,'r') as f:
        for line in f:
            line=line.strip().split()
            bedlabel.append(line[0])
...
def bedDinucCenter(bedfile,fasta,windows,up,down,pattern):
    dinucList=[]
    bedf=open(bedfile,'r')
    dinucCount=0
    for line in bedf:
...
#         out_matrix=open(outf[1],"w")
        ...
        for Bed in range(len(bedlist)):
            for dinuc,pattern in zip(Dinuc,Pattern):
                values=bedDinucCenter(bedlist[Bed],fa,windows,up,down,pattern)
                genomefrequency=GenomeFrequency(fa,pattern)
                tup1=np.array(values)
                tup1=tup1.astype(float)
                tup2=tup1.mean(axis=0)
    ...
    Xticklabel=["-"+L+"kb","start","end",R+"kb"]
    Ylabel="Relative dinucleotide density"
    plotfacet(dataframe,Xticks,Xticklabel,Ylabel,figurename)
else:
    pass
