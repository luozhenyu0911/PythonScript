...
args=commandArgs(T)
Db <- loadDb(args[1])
x1 = annotatePeak(args[2], tssRegion=c(-1000,0),level="gene",overlap="all",TxDb=Db,assignGenomicAnnotation = TRUE,
                  genomicAnnotationPriority = c("Promoter","5UTR", "3UTR", "Exon", "Intron","Downstream", "Intergenic"))
num <- nrow(read.table(args[2]))
write.table(x1,args[3],sep="\t",row.names = F)

#下游1kb以外的替换为Intergenic
as.data.frame(x1)$annotation %>% 
 gsub("^(Intron).*",'\\1',.) %>% 
 gsub("^(Exon).*",'\\1',.) %>% 
 gsub("Downstream \\(<1kb\\)","Downstream \\(<=1kb\\)",.) %>%
 gsub("Downstream \\(1\\-2kb\\)","Distal Intergenic",.) %>% 
 gsub("Downstream \\(2\\-3kb\\)","Distal Intergenic",.) %>% 
 gsub("Promoter","Promoter \\(<=1kb\\)",.) %>%
...
p1<-ggplot(df1, aes(x = 1, weight = value, fill = Group)) +
  ...
  scale_fill_manual(values=c("Promoter (<=1kb)"="#2980b9",
                             "5' UTR"="#9b59b6",
                             "3' UTR"="#e74c3c",
                             "Exon"="#16a085",
                             "Intron"="#f1c40f",
                             "Downstream (<=1kb)"="#e67e22",
                             "Distal Intergenic"="#34495e"),
                    limits=c("Promoter (<=1kb)",
                             "5' UTR",
                             "3' UTR",
                             "Exon",
                             "Intron",
