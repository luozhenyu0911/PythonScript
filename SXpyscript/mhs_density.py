...
def bamInfo(Input):
    ...
    for value in ReadInfo.values():
        ...
        if str(cutPoint1) in region[chrom]:
            region[chrom][str(cutPoint1)]+=2
        else:
            region[chrom][str(cutPoint1)]=2
        if str(cutPoint2) in region[chrom]:
            region[chrom][str(cutPoint2)]+=2
        else:
            region[chrom][str(cutPoint2)]=2
    return region
...
if "-b" in allopt:
    ...
    figure=[]
    if "-c" in allopt:
        inputvalues=bamtocount(bedpos,inIn,step_f)
    if "-f" in allopt:
        genevalues=bamtocount(bedpos,bamIn,step_f)
        SortId=[]
        FpkmValue=[]
        legendname=[]
...
if "--grouprev" in allopt:
    if '-B' in allopt:
        ...
        for line2 in bamlist:
            bamdir=line2.rstrip()
            genevalues=bamtocount(bedpos,bamdir,step_f)
        ...
        if "-C" in allopt:
            ...
            for line2 in inputlist:
                ...
                inputvalues=bamtocount(bedpos,inputdir,step_f)
