...
#读入FPKM数据,可以先对FPKM聚类，作图时再Z-score。
expr=read.delim('data/kmeans-exp.txt',header=T,row.names=1,sep="\t")


#=======================选K值：1.使用轮廓系数=======================

K <- 2:10  #设置遍历的K值范围，从2到10
round <- 20  # 实际分析设置为20以上，迭代计算取均值，避免局部最优
#计算每个K分类的轮廓系数
...
#否则每次运行，聚类结果都不同。
result=kmeans(expr,Selected_K)  #使用选择的K值进行kmeans聚类


#================设置用于展示的表达矩阵=====================
for_plot=data.frame(t(scale(t(expr))))  #使用log2转换缩小显示范围
...
#设置指示条
ha = rowAnnotation(df = for_plot$Group,
                   width = unit(5, "mm"),col=list(Group=setNames(rainbow(Selected_K),levels(for_plot$Group))))
...
g<-ggplot(result,aes(x =  variable , y = value , group = ID)) + 
  geom_line(alpha = 0.1 , aes(col =lineColors))


#对每个cluster添加一个中位数线
g<-g + stat_summary(fun.y=median, colour="black", 
                    geom="line",group=1)
