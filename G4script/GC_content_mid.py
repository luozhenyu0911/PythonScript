...
parser = argparse.ArgumentParser(description="This tool used to caculate GC content for genome",formatter_class = argparse.RawTextHelpFormatter)
...
parser.add_argument('--outfile','-o',type = str,help="-o <str>    Output GCcontent file",required = True)
...
def GCValue(bedfile,wdsize,fastafile,up,down,strand):
    fa = Fasta(fastafile)
    f = open(bedfile,"r")
    left=int(0)
    medium=int(up/wdsize)
    right=int(down/wdsize)+medium
    alllist = []
    for Line in f:
        L = []
        R = []
...
figurelist=[]
left=int(0)
medium=int(args.upstream/args.wdsize)
right=int(args.downstream/args.wdsize)+medium

for bed in bedlist:
    Alllist = GCValue(bed,args.wdsize,args.fastafile,args.upstream,args.downstream,args.strand)
    tup=np.array(Alllist)    
    tup2=tup.mean(axis=0)
    tup1=list(tup2)
    figurelist.append(tup1)
print("region","model","value","strand","type",sep="\t",end="\n",file=of)
N = 0
for lis in figurelist:
    n = 0
    for l in lis:
        print(n*int(args.wdsize),legend[N],l,args.strand,"GCcontent",sep="\t",end="\n",file=of)
        n += 1
    N += 1
for line in figurelist:
    plotsmooth(line)
plt.legend(legend)
plt.ylabel("GC content level")
plt.xticks([left,medium,right],["-"+str(args.upstream)+"bp","Midpoint",str(args.downstream)+"bp"])
