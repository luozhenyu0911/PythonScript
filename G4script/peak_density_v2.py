            ...
            for k in range(int(left),int(right),wdsize):
                for j in range(k,(k+wdsize)):
                    ...
                    if str(j) in readdict[chr]:
                        ...
                        pass
                # print((sumReads)/(count*wdsize)*10000)
                sublist.append(sumReads/(count*wdsize)*100000)
                sumReads=0
            Lis.append(sublist)
        else:
...
if referencePoint:
    ...
    if referencePoint=="start":
        ...
        for Bam in range(len(bamlist)):
            bamf=open(bamlist[Bam],'r')
            bamTotalReads=get_count(bamf)
            # print(bamTotalReads)
            bamf=open(bamlist[Bam],'r')
            bamrCoverageDict=bamTodict(bamf)
            # for i in bamrCoverageDict.values():
                # print(i)
        ...
        print("Bamlabel","Bedlabel","bin","value",sep="\t",end="\n",file=out_profile)
        for Bam in range(len(bamlist)):
            bamf=open(bamlist[Bam],format[args.format])
            bamTotalReads=get_count(bamf)
            bamf=open(bamlist[Bam],format[args.format])
            bamrCoverageDict=bamTodict(bamf)
            for Bed in range(len(bedlist)):
